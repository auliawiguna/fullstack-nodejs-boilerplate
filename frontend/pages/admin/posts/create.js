...
import { create } from '@validations/post'
...
const CreatePage = (props) => {
    ...
    const back = () => { return backDialog(router, "/admin/posts") }

    const submit = async (values) => {
        axios.defaults.headers.common['Authorization'] = `Bearer ${session.accessToken}`
        let url = `${process.env.NEXT_PUBLIC_API_URL}/api/v1/posts`
        try {
            setIsLoading(true)            
        ...
        <>
            ...
            <DashboardLayout title="Create Post">
                <Breadcrumb mb={4} fontWeight='medium' fontSize='sm'>
                    ...
                    <BreadcrumbItem>
                        <Link scroll={false} href='/admin/posts'>Posts</Link>
                    </BreadcrumbItem>

                    <BreadcrumbItem isCurrentPage>
                        <Link  scroll={false} href='#'>Create Posts</Link>
                    </BreadcrumbItem>
                </Breadcrumb>
                <div>
                            ...
                            >
                                <FormControl isRequired>
                                    ...
                                    <Input
                                        ...
                                        {...register('title')}
                                    />
                                    <Text color={'red'} className="invalid-feedback">{errors.title  ?.message}</Text>
                                </FormControl>
